...
Active. **Documentation mirror track FULLY CLOSED (2026-06-21):** deployed and verified; under git on a PRIVATE GitHub remote (`millsio/klaw-konfig`) via a repo-scoped deploy key; nightly cron at 3:15 AM CONFIRMED firing unattended; ETag caching verified working; both agents pointed at the mirror (AGENTS.md, CLAUDE.md). Mirror is closed but kept under light ongoing monitoring (see Documentation note). Remote access established 2026-06-17: Tailscale mesh + SSH working (VM passwordless, Windows hosts by password); RDP planned. **Active task now: security hardening** (now explicitly includes an OpenClaw-application layer, not just OS-level — see priority #5). Other near-term: link Claude Code to Mem MCP. **Forward track: the 5090 second node (build via VHDX seeding) and multi-node co-orchestration design — see the Networking & Infrastructure and Model Strategy & Orchestration notes. Planned workstreams: the Documentation Digest (whole-corpus distillation, WEEKLY, Claude Code + Opus) and the Debate Council (adversarial qwen-vs-Gemma review with an Opus judge + agenda-setter) — see the Documentation and Model Strategy notes.**
...
- Continuity layer is Mem (OPENCLAW PROJECT collection), NOT chat history — Claude Code cannot read claude.ai conversations, so durable project state must live in Mem.
...
- **No unattended Claude Code automation until strict cost-control protections are in place (decided 2026-06-27).** Applies to BOTH frontier-via-CC workstreams (Documentation Digest, Mem Collection Integrity check). Until cost guards exist, run these manually/attended only. Ties to the standing wish for plan-usage visibility (CC has no way to read remaining Max/OAuth balance today).
...
### 4. Link Claude Code to Mem MCP

- [ ] Add Mem MCP server to Claude Code (`claude mcp add`, streamable-http, `https://mcp.mem.ai/mcp`, plus Mem auth header/token). Confirm flag syntax via `--help`.
- [ ] Verify Claude Code can read the OPENCLAW PROJECT MASTER note.
- [ ] Fold the "read MASTER note first" session-start convention into the CLAUDE.md above.
...
- [ ] Move Mem API key from literal header to OpenClaw secrets store (SecretRef). Eliminates the `openclaw mcp doctor` warning. (Note: Mem's static bearer token CAN be SecretRef'd via `tokenRef`; OAuth credentials cannot — hard fail by the OAuth SecretRef Policy Guard.)
...
### Documentation Digest (whole-corpus distillation) — WEEKLY
...
### Mem Collection Integrity / Synthesis Check — SCHEDULED (cost-gated) — added 2026-06-27

- Sibling concept to the Documentation Digest's analysis pass, but pointed at the **OPENCLAW PROJECT Mem collection** rather than the docs corpus. Purpose is **validation + data integrity**, not compression: the collection is already hand-curated and small enough to load whole, so a compression digest adds little — the value is a periodic whole-collection synthesis/consistency pass.
- **What it does:** catch contradictions and stale facts across notes (the collection already carries several "CONTRADICTED IN PRACTICE" annotations); enforce the MASTER summary-synchronization rule (subject-note summaries vs MASTER drift); surface cross-note connections and orphaned open-items.
- **Engine:** Claude Code + Opus (same subscription-OAuth path as the Documentation Digest). The collection fits one Opus context → single-pass (no chunking needed).
- **Output = PROPOSED reconciliations for Brian's approval.** No autonomous Mem writes — fits the Mem write-permission gate (Fix A) now in the agent AGENTS.md files.
- **Cadence:** on a schedule, deliberately not over-frequent.
- **HARD GATE:** same cost-control gate as the Documentation Digest — do NOT automate any Claude Code run until strict cost-control protections are in place. Manual/attended runs only until then.

### Mem Scope Enforcement — MCP Filtering Proxy (defense-in-depth) — ENHANCEMENT — added 2026-06-27

- **Current guard (Fix A, implemented 2026-06-27):** per-agent AGENTS.md policy. Reads limited to the OPENCLAW PROJECT collection; NO Mem writes without Brian's explicit per-update approval (drafts to a local `mem-pending.md` when Brian is unavailable). Applied to `main` (Klaw) and `gemma-telegram` (Kettle); `gemma-amethyst` (Debater-B) is set to never write. This policy layer holds the line for now.
- **Token-level scoping (Fix B option a) — investigated, appears UNAVAILABLE:** mem.ai's published API docs show only standard account-wide Bearer auth; no documented collection-scoped API keys (checked docs.mem.ai create-note ref + the get.mem.ai API/MCP guide, 2026-06-27). NOT 100% certain without checking the account key-creation UI or asking Mem support.
- **Defense-in-depth path (Fix B option b):** a thin MCP proxy that injects the OPENCLAW PROJECT collection filter on every Mem call (physically prevents reads/writes outside the collection), OR a separate Mem account dedicated to OpenClaw. Proxy keeps one account but is more work; dedicated account is simpler but splits notes. Motivation is concrete: the single token currently reaches Brian's personal/financial notes.
- **Priority:** enhancement — the Fix A policy layer is the active guard; the proxy is the harder physical backstop for later.
...
Brian has \~1 TB free on OneDrive. Idea: periodically back up beyond the in-VM Mem snapshots. Candidates: Hyper-V VM snapshots/exports (large; host-side on Windows -> a host-side scheduled export/copy to OneDrive is the natural mechanism) and/or the small stuff (git repo, `mem-backups/`, configs). Not yet designed/built — captured so it isn't forgotten.
